...
class BackgroundService : Service() {
        ...
        when (intent?.action) {
            "START_AUDIO_CAPTURE" -> {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                    ...
                    if (data != null) {
                        audioCaptureHelper = AudioCaptureHelper(this)
                        audioCaptureHelper?.start(resultCode, data)
                    }
                }
            }
            "STOP_AUDIO_CAPTURE" -> {
                ...
                if (NotificationHelper.isPersistentNotificationEnabled(this)) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                        startForeground(FOREGROUND_NOTIFICATION_ID, createForegroundNotification(), ServiceInfo.FOREGROUND_SERVICE_TYPE_CONNECTED_DEVICE or ServiceInfo.FOREGROUND_SERVICE_TYPE_MEDIA_PROJECTION)
                    } else {
    ...
    companion object {
        ...
        val UPDATE_IMMUTABLE_FLAGS = PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
